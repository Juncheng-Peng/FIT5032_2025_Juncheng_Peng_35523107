...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="author in authors" :key="author.id"  :class="{ 'author-highlight': author.name === 'George Orwell' }">
      ...
      <!-- Activity 7: Render a list containing authors born after 1850. Hint: Make use of the v-for directive to iterate through the array of authors that you have filtered out. -->
       <ul>
        <li v-for="author in modernAuthors" :key="author.id"  :class="{ 'author-highlight': author.name === 'George Orwell' }"> 
          {{ author.name }} ({{ author.birthYear }})
        </li>
      </ul>
    ...
    <section class="lab-section">
      ...
      <p>We operate in:</p>
      <ul>
        <li v-for="(location, index) in bookstores.countries" :key="index">
          {{ location }}
        </li>
      </ul>
      <p>Our #1 seller:</p>
...
console.log("modernAuthors:",modernAuthors.value);
// Activity 3: Get all famous works
const allFamousWorks = computed(() =>
  authors.flatMap((author) => author.famousWorks.map((work) => work.title))
);
console.log("allFamousWorks:",allFamousWorks.value);
// Activity 4: Find author by name
const searchName = ref("George Orwell");
const orwell = computed(() => 
  // TODO: CODE TO FIND AUTHOR BY NAME HERE
  authors.find((author) => author.name === searchName.value)
)
console.log("orwell:",orwell.value);
// Activity 5: Find author by ID
const austen = computed(() => {
  // TODO: CODE TO FIND AUTHOR BY ID HERE
  return authors.find((author) => author.id === 1)
})
console.log("austen:",austen.value);
